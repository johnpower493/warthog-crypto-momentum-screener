"""
Bybit Order Book WebSocket Stream with Wall Detection
...
Topic: orderbook.50.<symbol> or orderbook.200.<symbol>
...
class OrderBookState:
    ...
    def update_from_snapshot(self, bids: List[List[str]], asks: List[List[str]], seq: int = 0):
    ...
    def update_from_delta(self, bids: List[List[str]], asks: List[List[str]], seq: int = 0):
        ...
        while i < len(sorted_walls):
            ...
            if len(cluster_walls) == 1:
                ...
                clustered.append({
                    ...
                    "strength": round(avg_strength * len(cluster_walls), 2),
                    "distance_pct": round(distance_pct, 2),
                    "is_cluster": True,
...
async def stream_orderbook(symbol: str, depth_levels: int = 50) -> AsyncIterator[dict]:
    """Stream order book updates for a Bybit symbol.
    
    Yields normalized updates with wall detection.
    
    Args:
        symbol: Trading pair (e.g., "BTCUSDT")
        depth_levels: Number of levels to subscribe to (1, 50, 200, or 500)
    """
    topic = f"orderbook.{depth_levels}.{symbol}"
    url = BYBIT_WS_LINEAR
    ...
    while True:
        try:
            attempt += 1
            log.info(f"Bybit orderbook WS connect {symbol} (attempt {attempt})")
                ...
                async for message in ws:
                    try:
                        ...
                        if data.get("op") == "subscribe":
                            if data.get("success"):
                                log.info(f"Bybit orderbook subscribed: {topic}")
                            else:
                                log.warning(f"Bybit orderbook subscription failed: {data}")
                            continue
                        ...
                        if msg_type == "snapshot":
                            state.update_from_snapshot(bids, asks, seq)
                        elif msg_type == "delta":
                            state.update_from_delta(bids, asks, seq)
                        else:
                            continue
                        ...
                        log.debug(f"Bybit orderbook parse error: {e}")
                        continue
                        
        except asyncio.CancelledError:
            raise
        except Exception as e:
            log.warning(f"Bybit orderbook WS error {symbol}: {type(e).__name__}: {e} (reconnect {backoff:.1f}s)")
...
async def run_orderbook_collector(symbol: str) -> None:
    ...
    """
    log.info(f"Starting Bybit orderbook collector for {symbol}")
    ...
    try:
        ...
        log.info(f"Bybit orderbook collector cancelled: {symbol}")
        raise
    except Exception as e:
        log.error(f"Bybit orderbook collector error {symbol}: {e}")
        raise
